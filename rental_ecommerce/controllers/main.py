...
class WebsiteSaleForm(WebsiteForm):
    ...
    @http.route('/website_form/rent.sale.order', type='http', auth="public", methods=['POST'], website=True)
...
class Website(Website):
    ...
    def get_switchable_related_views(self, key):
        ...
        if key == 'rental_ecommerce.product':
            if not request.env.user.has_group('product.group_product_variant'):
                view_product_variants = request.website.viewref('rental_ecommerce.product_variants')
    ...
    def toggle_switchable_view(self, view_key):
        ...
        if view_key in ('rental_ecommerce.products_list_view', 'rental_ecommerce.add_grid_or_list_option'):
...
class WebsiteSale(http.Controller):
    ...
    def sitemap_rent(env, rule, qs):
        if not qs or qs.lower() in '/rent':
            yield {'loc': '/rent'}

        Category = env['product.public.category']
        dom = sitemap_qs2dom(qs, '/rent/category', Category._rec_name)
        dom += env['website'].get_current_website().website_domain()
        for cat in Category.search(dom):
            loc = '/rent/category/%s' % slug(cat)
    ...
    @http.route([
        '''/rent''',
        '''/rent/page/<int:page>''',
        '''/rent/category/<model("product.public.category"):category>''',
        '''/rent/category/<model("product.public.category"):category>/page/<int:page>'''
    ], type='http', auth="public", website=True, sitemap=sitemap_rent)
    def rent(self, page=0, category=None, search='', ppg=False, **post):
        ...
        keep = QueryURL('/rent', category=category and int(category), search=search, attrib=attrib_list, order=post.get('order'))
        ...
        if category:
            url = "/rent/category/%s" % slug(category)
        ...
        if not layout_mode:
            if request.website.viewref('rental_ecommerce.products_list_view').active:
                layout_mode = 'list'
            else:
                layout_mode = 'grid'
        ...
        return request.render("rental_ecommerce.products", values)

    @http.route(['/rent/<model("product.template"):product>'], type='http', auth="public", website=True, sitemap=True)
    def product(self, product, category='', search='', **kwargs):
        if not product.can_access_from_current_website():
            raise NotFound()

        return request.render("rental_ecommerce.product", self._prepare_product_values(product, category, search, **kwargs))

    @http.route(['/rent/product/<model("product.template"):product>'], type='http', auth="public", website=True, sitemap=False)
    def old_product(self, product, category='', search='', **kwargs):
        # Compatibility pre-v14
        return request.redirect(_build_url_w_params("/rent/%s" % slug(product), request.params), code=301)
    ...
    def _prepare_product_values(self, product, category, search, **kwargs):
        ...
        keep = QueryURL('/rent', category=category and category.id, search=search, attrib=attrib_list)
        ...
        view_track = request.website.viewref("rental_ecommerce.product").track
    ...
    @http.route(['/rent/change_pricelist/<model("product.pricelist"):pl_id>'], type='http', auth="public", website=True, sitemap=False)
    def pricelist_change(self, pl_id, **post):
        ...
        return request.redirect(request.httprequest.referrer or '/rent')

    @http.route(['/rent/pricelist'], type='http', auth="public", website=True, sitemap=False)
    def pricelist(self, promo, **post):
        redirect = post.get('r', '/rent/cart')
    ...
    @http.route(['/rentorder'], type='http', auth="public", website=True, sitemap=False, csrf=False)
    def rentorder(self, **post):
        Product_template = request.env['product.template'].search([('id','=',post.get('product'))])
        sale_order = request.env['sale.order'].create({'partner_id':request.env.user.partner_id.id,
                                                       'is_rental_order':True})
        sale_order_line = request.env['sale.order.line'].create({'product_id':Product_template.product_variant_ids.id,
        ...
        # rental_wizard = request.env['rental.wizard'].create({'rental_order_line_id':sale_order_line.id,
        ...
        #
        #                                                      })
        return request.render("rental_ecommerce.confirmation_rental", {'order': sale_order})

    @http.route(['/rent/cart'], type='http', auth="public", website=True, sitemap=False)
    def cart(self, access_token=None, revive='', **post):
        ...
        if access_token:
            ...
            if abandoned_order.state != 'draft':  # abandoned cart already finished
                ...
                return request.redirect('/rent/cart')
            elif revive == 'merge':
        ...
        if post.get('type') == 'popover':
            ...
            return request.render("rental_ecommerce.cart_popover", values, headers={'Cache-Control': 'no-cache'})

        return request.render("rental_ecommerce.cart", values)

    @http.route(['/rent/cart/update'], type='http', auth="public", methods=['POST'], website=True)
    def cart_update(self, product_id, add_qty=1, set_qty=0, **kw):
        ...
        if kw.get('express'):
            return request.redirect("/rent/checkout?express=1")

        return request.redirect("/rent/cart")

    @http.route(['/rent/cart/update_json'], type='json', auth="public", methods=['POST'], website=True, csrf=False)
    def cart_update_json(self, product_id, line_id=None, add_qty=None, set_qty=None, display=True):
        ...
        value['rental_ecommerce.cart_lines'] = request.env['ir.ui.view']._render_template("rental_ecommerce.cart_lines", {
            'website_sale_order': order,
            'date': fields.Date.today(),
            'suggested_products': order._cart_accessories()
        })
        value['rental_ecommerce.short_cart_summary'] = request.env['ir.ui.view']._render_template("rental_ecommerce.short_cart_summary", {
            'website_sale_order': order,
        })
        return value

    @http.route('/rent/save_shop_layout_mode', type='json', auth='public', website=True)
    ...
    def checkout_redirection(self, order):
        ...
        if tx and tx.state != 'draft':
            return request.redirect('/rent/payment/confirmation/%s' % order.id)
        ...
        for k, v in values.items():
            ...
            if k in authorized_fields and v is not None:
                ...
                if k not in ('field_required', 'partner_id', 'callback', 'submitted'): # classic case
                    _logger.debug("rental_ecommerce postprocess: %s value has been dropped (empty or not writable)" % k)
    ...
    @http.route(['/rent/address'], type='http', methods=['GET', 'POST'], auth="public", website=True, sitemap=False)
    def address(self, **kw):
        ...
        # IF ORDER LINKED TO A PARTNER
            ...
            if partner_id > 0:
                ...
                return request.redirect('/rent/checkout')
        ...
        if 'submitted' in kw:
            ...
            if errors:
                ...
                if mode[1] == 'billing':
                    ...
                    if not kw.get('use_same'):
                        kw['callback'] = kw.get('callback') or \
                            (not order.only_services and (mode[0] == 'edit' and '/rent/checkout' or '/rent/address'))
                ...
                if not errors:
                    return request.redirect(kw.get('callback') or '/rent/confirm_order')
        ...
        return request.render("rental_ecommerce.address", render_values)

    @http.route(['/rent/checkout'], type='http', auth="public", website=True, sitemap=False)
    def checkout(self, **post):
        ...
        if order.partner_id.id == request.website.user_id.sudo().partner_id.id:
            return request.redirect('/rent/address')

        for f in self._get_mandatory_billing_fields():
            if not order.partner_id[f]:
                return request.redirect('/rent/address?partner_id=%d' % order.partner_id.id)

        values = self.checkout_values(**post)

        if post.get('express'):
            return request.redirect('/rent/confirm_order')
        ...
        if post.get('xhr'):
            return 'ok'
        return request.render("rental_ecommerce.checkout", values)

    @http.route(['/rent/confirm_order'], type='http', auth="public", website=True, sitemap=False)
    def confirm_order(self, **post):
        ...
        extra_step = request.website.viewref('rental_ecommerce.extra_info_option')
        if extra_step.active:
            return request.redirect("/rent/extra_info")

        return request.redirect("/rent/payment")

    # ------------------------------------------------------
    # Extra step
    # ------------------------------------------------------
    @http.route(['/rent/extra_info'], type='http', auth="public", website=True, sitemap=False)
    def extra_info(self, **post):
        ...
        return request.render("rental_ecommerce.extra_info", values)
    ...
    def _get_shop_payment_values(self, order, **kwargs):
        values = dict(
            ...
            return_url= '/rent/payment/validate',
    ...
    @http.route(['/rent/payment'], type='http', auth="public", website=True, sitemap=False)
    def payment(self, **post):
        ...
        return request.render("rental_ecommerce.payment", render_values)

    @http.route(['/rent/payment/transaction/',
        '/rent/payment/transaction/<int:so_id>',
        '/rent/payment/transaction/<int:so_id>/<string:access_token>'], type='json', auth="public", website=True)
    ...
    @http.route('/rent/payment/token', type='http', auth='public', website=True, sitemap=False)
    def payment_token(self, pm_id=None, **kwargs):
        ...
        if not order:
            return request.redirect('/rent/?error=no_order')
        ...
        try:
            ...
            return request.redirect('/rent/?error=invalid_token_id')

        # We retrieve the token the user want to use to pay
        if not request.env['payment.token'].sudo().search_count([('id', '=', pm_id)]):
            return request.redirect('/rent/?error=token_not_found')

        # Create transaction
        vals = {'payment_token_id': pm_id, 'return_url': '/rent/payment/validate'}

        tx = order._create_payment_transaction(vals)
        PaymentProcessing.add_payment_transaction(tx)
        return request.redirect('/payment/process')

    @http.route('/rent/payment/get_status/<int:sale_order_id>', type='json', auth="public", website=True)
    def payment_get_status(self, sale_order_id, **post):
        ...
        return {
            ...
            'message': request.env['ir.ui.view']._render_template("rental_ecommerce.payment_confirmation_status", {
                'order': order
            })
        }

    @http.route('/rent/payment/validate', type='http', auth="public", website=True, sitemap=False)
    def payment_validate(self, transaction_id=None, sale_order_id=None, **post):
        ...
        return request.redirect('/rent/confirmation')

    @http.route(['/rent/terms'], type='http', auth="public", website=True, sitemap=True)
    def terms(self, **kw):
        return request.render("rental_ecommerce.terms")

    @http.route(['/rent/confirmation'], type='http', auth="public", website=True, sitemap=False)
    def payment_confirmation(self, **post):
        ...
        if sale_order_id:
            order = request.env['sale.order'].sudo().browse(sale_order_id)
            return request.render("rental_ecommerce.confirmation", {'order': order})
        else:
            return request.redirect('/rent')

    @http.route(['/rent/print'], type='http', auth="public", website=True, sitemap=False)
    ...
    @http.route(['/rent/tracking_last_order'], type='json', auth="public")
    ...
    @http.route(['/rent/add_product'], type='json', auth="user", methods=['POST'], website=True)
    ...
    @http.route(['/rent/change_sequence'], type='json', auth='user')
    ...
    @http.route(['/rent/change_size'], type='json', auth='user')
    def change_size(self, id, x, y):
        product = request.env['product.template'].browse(id)
        return product.write({'website_size_x': x, 'website_size_y': y})

    @http.route(['/rent/change_ppg'], type='json', auth='user')
    def change_ppg(self, ppg):
        request.env['website'].get_current_website().shop_ppg = ppg

    @http.route(['/rent/change_ppr'], type='json', auth='user')
    ...
    @http.route(['/rent/country_infos/<model("res.country"):country>'], type='json', auth="public", methods=['POST'], website=True)
    ...
    @http.route('/rent/products/autocomplete', type='json', auth='public', website=True)
    ...
    # --------------------------------------------------------------------------
    @http.route('/rent/products/recently_viewed', type='json', auth='public', website=True)
    ...
    def _get_products_recently_viewed(self):
        ...
        if visitor:
            ...
            if products_ids:
                ...
                rating = request.website.viewref('rental_ecommerce.product_comment').active
                res = {'products': []}
    ...
    @http.route('/rent/products/recently_viewed_update', type='json', auth='public', website=True)
    ...
    @http.route('/rent/products/recently_viewed_delete', type='json', auth='public', website=True)
    ...
    @http.route('/rental_ecommerce/snippet/options_filters', type='json', auth='user', website=True)

odoo.define('rental_ecommerce.tracking', function (require) {
...
publicWidget.registry.websiteSaleTracking = publicWidget.Widget.extend({
    ...
    events: {
        'click form[action="/rent/cart/update"] a.a-submit': '_onAddProductIntoCart',
        'click a[href="/rent/checkout"]': '_onCheckoutStart',
        'click div.oe_cart a[href^="/web?redirect"][href$="/rent/checkout"]': '_onCustomerSignin',
        'click form[action="/rent/confirm_order"] a.a-submit': '_onOrder',
    ...
    start: function () {
        ...
        // ...
        if (this.$('div.oe_rental_ecommerce_tx_status').length) {
            this._trackGA('require', 'ecommerce');

            var orderID = this.$('div.oe_rental_ecommerce_tx_status').data('order-id');
            this._vpv('/stats/ecom/order_confirmed/' + orderID);

            this._rpc({
                route: '/rent/tracking_last_order/',
            }).then(function (o) {

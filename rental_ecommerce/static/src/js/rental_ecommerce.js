odoo.define('rental_ecommerce.cart', function (require) {
'use strict';
...
publicWidget.registry.websiteSaleCartLink = publicWidget.Widget.extend({
    selector: '#top_menu a[href$="/rent/cart"]',
    events: {
    ...
    _onMouseEnter: function (ev) {
        ...
        timeout = setTimeout(function () {
            ...
            self._popoverRPC = $.get("/rent/cart", {
                type: 'popover',
            }).then(function (data) {
...
odoo.define('rental_ecommerce.rental_ecommerce_category', function (require) {
'use strict';
...
odoo.define('rental_ecommerce.rental_ecommerce', function (require) {
'use strict';
...
publicWidget.registry.WebsiteSale = publicWidget.Widget.extend(VariantMixin, {
    ...
    _changeCartQuantity: function ($input, value, $dom_optional, line_id, productIDs) {
        ...
        this._rpc({
            ...
            if (!data.cart_quantity) {
                return window.location = '/rent/cart';
            }
    ...
    _changeCountry: function () {
        ...
        this._rpc({
            route: "/rent/country_infos/" + $("#country_id").val(),
            params: {
    ...
    _submitForm: function () {
        ...
        return wUtils.sendRequest('/rent/cart/update', params);
    },
    /**
     * @private
     * @param {MouseEvent} ev
...
publicWidget.registry.WebsiteSaleLayout = publicWidget.Widget.extend({
    ...
    _onApplyShopLayoutChange: function (ev) {
        ...
        if (!this.editableMode) {
            this._rpc({
                route: '/rent/save_shop_layout_mode',
                params: {
...
publicWidget.registry.websiteSaleCart = publicWidget.Widget.extend({
    selector: '.oe_rental_ecommerce .oe_cart',
    events: {
    ...
    _onClickEditAddress: function (ev) {
        ev.preventDefault();
        $(ev.currentTarget).closest('div.one_kanban').find('form.d-none').attr('action', '/rent/address').submit();
    },
    /**
     * @private
     * @param {Event} ev
